...
function rsa_chiffre_pour () {
    openssl pkeyutl -encrypt -inkey  "${1}.pem" -pubin |  to_base64 | remove_newlines
}
function rsa_dechiffre () {
    from_base64 | openssl pkeyutl -decrypt -inkey clef_privee.pem
}

function rsa_signe () {
    openssl pkeyutl -sign -inkey clef_privee.pem | to_base64
}
function rsa_signature_de () {
    from_base64 | openssl pkeyutl -verify -inkey "${1}.pem"  -pubin
}
